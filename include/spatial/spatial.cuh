...
__global__ void scale_and_convert_to_half_kernel(const inputT *d_input, const scaleT *d_scale, outputT *d_output)  {
    ...
    int val = static_cast<int>(d_input[0][channel_idx][packet_idx][time_idx][receiver_idx][polarization_idx][complex_idx]);
    int scale_factor = static_cast<int>(d_scale[0][channel_idx][packet_idx][receiver_idx][polarization_idx]);

    int result = val * scale_factor;
    d_output[0][channel_idx][packet_idx][time_idx][receiver_idx][polarization_idx][complex_idx] = __int2half_rn(result);
};
template<typename inputT, typename scaleT, typename outputT, size_t NR_CHANNELS, size_t NR_POLARIZATIONS, size_t NR_RECEIVERS, size_t NR_TIME_STEPS_PER_PACKET, size_t NR_PACKETS> 
void scale_and_convert_to_half(const inputT *d_input, const scaleT *d_scale, outputT *d_output, cudaStream_t stream){
    ...
    (scale_and_convert_to_half_kernel<inputT, scaleT, outputT, NR_CHANNELS, NR_POLARIZATIONS, NR_RECEIVERS, NR_TIME_STEPS_PER_PACKET, NR_PACKETS>) <<<dim3(num_blocks_x, num_blocks_y, 1), dim3(num_threads_x,1, 1), 0, stream>>>(d_input, d_scale, d_output);
}
