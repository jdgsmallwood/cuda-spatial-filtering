...
// ENUPosition, ArrayLocation, FrequencyPlan, BeamTarget, and
// channel_to_frequency_hz() live in pointing.hpp (included via pipeline.hpp
// above) -- they are pure geometry/target-description types shared with
// compute_steering_weights(), which has no business depending on CommonArgs.
...
struct CommonArgs {
  ...
  // Antenna ENU positions (metres) relative to the array reference point,
  // keyed by absolute antenna ID -- parsed from config.json's
  // "antenna_positions" block. Populated with placeholder zeros until real
  // surveyed values are supplied there.
  std::unordered_map<int, ENUPosition> antenna_positions;
  // Lat/Long/height of the array's reference point, parsed from config.json's
  // "array_location" block (defaults to all-zero if absent).
  ArrayLocation array_location;
  // Channel-to-frequency mapping, parsed from config.json's "frequency_plan"
  // block (defaults to all-zero placeholders if absent -- see
  // channel_to_frequency_hz()).
  FrequencyPlan frequency_plan;
  // One pointing target per beam, parsed from --targets-filename. Empty when
  // no targets file is supplied -- callers should fall back to static
  // beam_weights/uniform-default behaviour in that case (steering is opt-in).
