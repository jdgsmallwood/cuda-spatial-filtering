...
TEST(CorrelateTest, SimpleTestExtended) {
  ...
  try {
    ...
    // >>> import numpy as np
    // >>> i = np.array([2 + 3j, 5 + 1j, -2 + 3j, 2 -3j, 1 + 1j, 0, 10 + 2j, 4 +
    // 4j])
    // >>> i2 = np.array([4 + 5j, 2 + 2j, 9 + 1j, -4 -5j, 0, 1 -1j, 2 + 2j, 2+
    // 2j])
    // >>> np.dot(i, i2.conj())
    //  (67+29j)
    // >>> np.dot(i, i.conj())
...
TEST(TestSpatial, TestRearrangeMatrixToCCGLIBFormat) {
  ...
  rearrange_matrix_to_ccglib_format<__half>(input_matrix, output_matrix, n_row,
                                            n_col, true);
...
TEST(TestCCGLIBTranspose, TestTransposeSimple) {
  ...
  cudaMemcpy(d_input, input_matrix,
             n_row * n_col * sizeof(std::complex<__half>), cudaMemcpyDefault);
  ...
  ccglib::transpose::Transpose transpose(
      batch_size, n_row, n_col, tile_size_x, tile_size_y, precision, cu_device,
      stream, ccglib::ComplexAxisLocation::complex_interleaved);

  transpose.Run((CUdeviceptr)d_input, (CUdeviceptr)d_output);

  cudaMemcpyAsync(output_matrix, d_output, n_row * n_col * sizeof(__half) * 2,
...
TEST(TestSpatial, TestRearrangeCCGLIBMatrixToCompactFormat) {
  ...
  rearrange_ccglib_matrix_to_compact_format(input_matrix, output_matrix, n_row,
                                            n_col);
