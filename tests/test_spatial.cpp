...
//       make_cuComplex(3.5, 0.0), make_cuComplex(0.5, 1.0),
//       make_cuComplex(0.0, 0.0), make_cuComplex(0.5, -1.0),
//       make_cuComplex(3.5, 0.0), make_cuComplex(0.0, 0.0),
//       make_cuComplex(0.0, 0.0), make_cuComplex(0.0, 0.0),
//       make_cuComplex(2.0, 0)};
//
//   float *h_eigenvalues;
...
//     // >>> import numpy as np
//     // >>> i = np.array([2 + 3j, 5 + 1j, -2 + 3j, 2 -3j, 1 + 1j, 0, 10 + 2j,
//     4 +
//     // 4j])
//     // >>> i2 = np.array([4 + 5j, 2 + 2j, 9 + 1j, -4 -5j, 0, 1 -1j, 2 + 2j,
//     2+
//     // 2j])
//     // >>> np.dot(i, i2.conj())
//     //  (67+29j)
...
//
//   checkCudaCall(
//       cudaMallocHost(&A, sizeof(__half) * 2 * n_row * n_col * batch_size));
//
//   __half(*B);
//
//   checkCudaCall(
//       cudaMallocHost(&B, sizeof(__half) * 2 * n_row * n_col * batch_size));
//
...
//
//   checkCudaCall(
//       cudaMallocHost(&C, sizeof(float) * 2 * n_row * n_col * batch_size));
//
...
//     std::cout << i << "i" << C[i] << std::endl;
//   }
//   // The first matrix "A" is row-major. The second matrix "B" is column-major
//   so
...
//
//   checkCudaCall(
//       cudaMallocHost(&A, sizeof(__half) * 2 * n_row * n_inner * batch_size));
//
//   __half(*B);
//
//   checkCudaCall(
//       cudaMallocHost(&B, sizeof(__half) * 2 * n_inner * n_col * batch_size));
//
...
//
//   input_matrix = (std::complex<__half> *)malloc(n_row * n_col *
//                                                 sizeof(std::complex<__half>));
...
//
//   rearrange_matrix_to_ccglib_format<__half>(input_matrix, output_matrix,
//   n_row,
//                                             n_col, true);
//
...
//   auto precision =
//   ccglib::Precision(ccglib::ValueType::float16).GetInputBits();
//
//   CUdevice cu_device;
...
//     input_matrix[i] =
//         std::complex<__half>(__float2half(1.0f), __float2half(i * 1.0f));
//   }
//   cudaMemcpy(d_input, input_matrix,
//              n_row * n_col * sizeof(std::complex<__half>),
//              cudaMemcpyDefault);
...
//       batch_size, n_row, n_col, tile_size_x, tile_size_y, precision,
//       cu_device, stream, ccglib::ComplexAxisLocation::complex_interleaved);
//
//   transpose.Run((CUdeviceptr)d_input, (CUdeviceptr)d_output);
//
//   cudaMemcpyAsync(output_matrix, d_output, n_row * n_col * sizeof(__half) *
//   2,
...
//   output_matrix = (std::complex<float> *)malloc(n_row * n_col *
...
//   input_matrix[7] = 4.0f;
//
//   rearrange_ccglib_matrix_to_compact_format(input_matrix, output_matrix,
//   n_row,
//                                             n_col);
//
//   for (int i = 0; i < n_row * n_col; ++i) {
